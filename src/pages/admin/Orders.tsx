  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">{t('orders')}</h1>
          <p className="text-slate-400 font-bold mt-1">{t('orders_desc')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: t('total_orders'), value: orders.length, icon: Package, color: 'text-blue-600', bg: 'bg-blue-100' },
          { label: t('processing'), value: orders.filter(o => o.status === 'قيد المعالجة' || o.status === 'Processing').length, icon: Clock, color: 'text-amber-600', bg: 'bg-amber-100' },
          { label: t('delivered'), value: orders.filter(o => o.status === 'تم التوصيل' || o.status === 'Delivered').length, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-100' },
          { label: t('cancelled'), value: orders.filter(o => o.status === 'ملغي' || o.status === 'Cancelled').length, icon: X, color: 'text-red-600', bg: 'bg-red-100' },
        ].map((stat, i) => (
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-2xl px-5 py-2">
            ...
            >
              ...
              <option value="قيد المعالجة">{t('processing')}</option>
              <option value="تم الشحن">{t('shipped')}</option>
              <option value="تم التوصيل">{t('delivered')}</option>
              <option value="ملغي">{t('cancelled')}</option>
            </select>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-right">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('order_number')}</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('customer')}</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('date')}</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('total')}</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest">{t('status')}</th>
                <th className="px-8 py-6 text-xs font-black text-slate-400 uppercase tracking-widest text-center">{t('action')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {isLoadingOrders ? (
                <tr>
                  <td colSpan={6} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-slate-400 font-black">{t('loading_orders')}</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={6} className="px-8 py-20 text-center">
                    <p className="text-slate-400 font-black">{t('no_orders')}</p>
                  </td>
                </tr>
              ) : (
                paginatedOrders.map((order, index) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-5 font-black text-brand-primary text-sm">{order.total_amount.toFixed(2)} {t('sar')}</td>
                    <td className="px-8 py-5">
                        ...
                        <div className="relative">
                          ...
                          <AnimatePresence>
                            {activeDropdown === order.id && (
                              ...
                              >
                                <div className="p-2">
                                  <p className="px-4 py-2 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">{t('change_status')}</p>
                                  {['قيد المعالجة', 'تم الشحن', 'تم التوصيل', 'ملغي'].map((status) => (
                                    ...
                                    >
                                      <span>
                                        {status === 'قيد المعالجة' ? t('processing') : 
                                         status === 'تم الشحن' ? t('shipped') : 
                                         status === 'تم التوصيل' ? t('delivered') : 
                                         status === 'ملغي' ? t('cancelled') : status}
                                      </span>
                                    </button>
                                  ))}
                                </div>
          ...
          >
            ...
            >
              <div className="p-8 border-b border-slate-100 flex justify-between items-center shrink-0">
                <div>
                  <h2 className="text-2xl font-black text-slate-800">{t('order_details')} {selectedOrder.id}</h2>
                  <p className="text-slate-400 font-bold text-sm mt-1">
                    {new Date(selectedOrder.created_at).toLocaleDateString(i18n.language === 'ar' ? 'ar-SA' : 'en-US')}
                  </p>
                </div>
              ...
              <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
                ...
                <div className={`p-6 rounded-[1.5rem] flex items-center justify-between ${getStatusColor(selectedOrder.status)}`}>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase tracking-wider opacity-70">{t('current_status')}</p>
                      <p className="text-xl font-black">
                        {selectedOrder.status === 'قيد المعالجة' ? t('processing') : 
                         selectedOrder.status === 'تم الشحن' ? t('shipped') : 
                         selectedOrder.status === 'تم التوصيل' ? t('delivered') : 
                         selectedOrder.status === 'ملغي' ? t('cancelled') : selectedOrder.status}
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest">{t('customer_info')}</h4>
                ...
                <div className="space-y-4">
                  <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest">{t('ordered_products')}</h4>
                    ...
                    {[1, 2].map((_, i) => (
                      <div key={i} className="flex items-center justify-between p-4 border border-slate-100 rounded-2xl">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-black text-slate-800">Product {i + 1}</p>
                            <p className="text-xs font-bold text-slate-400">{t('quantity')}: 1</p>
                          </div>
                        </div>
                        <p className="font-black text-slate-800 text-sm">125.00 {t('sar')}</p>
                      </div>
                    ))}
                  </div>
                </div>
